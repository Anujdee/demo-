...
<body
                            ...
                            <table>
                                <thead>
                                    <tr>
                                        ...
                                        <th class="ltr:rounded-l-md rtl:rounded-r-md">Plan Name  </th>
                                        <th>Payment</th>
                                        <th>Email Address</th>
                                ...
                                <tbody>
    <?php
    // Query to fetch the full row where plan_name has a value
    $query = mysqli_query($conn, "SELECT * FROM tbl_quotes WHERE plan_name IS NOT NULL AND plan_name != ''");
    while ($row = mysqli_fetch_array($query)) { 
    ?>
        <tr>
            <!-- Display the plan_name and pay_type columns -->
            <td class="font-semibold"><?php echo $row['plan_name']; ?></td>
            ...
            <td><?php echo $row['subscriptio']; ?></td>
            <td class="text-center">
                <a href="subscription-apply?state_id=<?php echo $row['CreatedAt']; ?>" onclick="return confirm('Are you sure you want to delete this details?');">
    ...
    if ($_GET['state_id']) {
        $a = $_GET['state_id'];
        $query = mysqli_query($conn, "DELETE FROM tbl_quotes WHERE CreatedAt='$a'");
        if ($query) {
            echo "<script>alert('Details has been deleted');
  window.location.href='subscription-apply';</script>";
        } else {
