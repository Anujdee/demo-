...
if (isset($_POST['submit'])) {
    ...
    $sql = mysqli_query($conn, "INSERT INTO `deals` (`bill`,`product`,`price`) VALUES ('$file_name','$pname','$pprice')");
    // if ($sql) {
    ...
    if (move_uploaded_file($tempname, $folder)) {
        echo "<script>alert('Your image has been submitted !');window.location.href='view-deals';</script>";
    } else {
                            ...
                            <template x-if="tab === 'home'">
                                <div>
                                    <form method="post" enctype="multipart/form-data" class="mb-5 rounded-md border border-[#ebedf2] bg-white p-4 dark:border-[#191e3a] dark:bg-[#0e1726]">
                                        <h6 class="mb-5 text-lg font-bold">Add New Deals</h6>
                                        <div class="flex flex-col sm:flex-row">

                                            <div class="grid flex-1 grid-cols-1 gap-5 sm:grid-cols-2">
                                                <div>
                                                    <label for="name">Deal Image</label>
